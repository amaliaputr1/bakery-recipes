...
function RecipeList({ searchQuery = "" }) {
  ...
  useEffect(() => {
    fetch("http://localhost/bakery-recipes-api/resep.php")
      .then((res) => res.json())
  ...
  const handleDelete = (id) => {
    if (window.confirm("Yakin mau hapus resep ini?")) {
      fetch(`http://localhost/bakery-recipes-api/resep.php?id=${id}`, {
        method: "DELETE",
      })
        .then((res) => res.json())
        .then((data) => {
  ...
  return (
    ...
    >
      <main className="container mx-auto p-6">
        {/* HEADER */}
        <div className="bg-white/70 backdrop-blur rounded-xl p-6 shadow-md mb-6">
          <div className="flex justify-between items-start border-b">
            <div>
              ...
              <p className="text-sm text-gray-500">
                Koleksi resep bakery yang tersedia
              </p>
            </div>
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex items-center gap-2 bg-gradient-to-r from-blue-500 to-indigo-600 text-white px-5 py-2.5 rounded-lg shadow-md hover:shadow-xl hover:scale-105 transition font-semibold"
              >
        ...
        {/* KATEGORI */}
        <div className="bg-white/70 backdrop-blur rounded-xl p-4 shadow-md mb-6">
